...
// ---- Live data config ----
// Point this at your deployed Vercel backend, e.g. "https://your-app.vercel.app"
const API_BASE = "https://YOUR-VERCEL-APP.vercel.app";

// Each NYC subway stop has a real GTFS "stop_id". For the full system, station
// names/locations/IDs now come live from /api/stations.js (backed by MTA's
// official data) instead of being hand-typed — see fetchNearbyStations below.
...
// The realtime feeds are split by line group, but a station can only be matched
// against one group correctly. To keep this simple for any station picked up
// from the full GTFS list (which doesn't tag which group it belongs to), we
// pull all groups in parallel once and merge them into a single stopId -> arrivals lookup.
...
// A small station graph: each station connects to others it's directly reachable
// from via one line, with an average travel time in minutes. This is a simplified
// stand-in for MTA's full station/track network — same idea Google Maps uses
// internally, just scoped down for this prototype. Extend STATION_GRAPH with more
// stations/edges to cover the full system.
const STATION_GRAPH = {
...
const TRANSFER_PENALTY_MINS = 4; // average wait + walk when switching lines

// Finds up to `limit` distinct routes between two stations, ranked by total time.
// Uses a simple weighted search (Dijkstra-style) over STATION_GRAPH, then a couple
// of alternate paths by temporarily penalizing the best path's first edge — enough
// to show "fastest" vs "fewer transfers" without needing a full k-shortest-paths lib.
function findRoutes(origin, destination, limit = 3) {
  ...
  function dijkstra(blockedFirstEdgeTo = null) {
    ...
    while (visited.size < Object.keys(STATION_GRAPH).length) {
      ...
      for (const edge of STATION_GRAPH[current].edges) {
        if (current === origin && edge.to === blockedFirstEdgeTo) continue; // force an alternate path
  ...
  // Try alternates by blocking the first leg of the best route so far
...
// ---- Mock data layer (fallback if live fetch fails, or while API_BASE is unset) ----
const LINE_COLORS = {
...
function RoutePlanner({ onNeedsPaywall }) {
  ...
  // Free tier: only routes that start AND end within the user's nearby radius
  // (Union Sq + 14 St 8 Av, in this prototype) are unlocked. Anything reaching
  // further into the city requires Transit+.
...
function StopRow({ stop, locked, onLocked, saved, onToggleSave, arrivalsData }) {
  ...
  // The full citywide station/stop list (from MTA's GTFS data) gives us names
  // and locations, but not which lines serve each one without extra mapping —
  // so when stop.lines isn't known upfront, we show line bullets from whatever
  // live arrivals come back instead, and a generic label until then.
...
export default function TransitApp() {
  ...
  // Step 1: get the user's real location, then pull every subway station and
  // bus stop within the free radius from MTA's full official list.
  useEffect(() => {
    let cancelled = false;
    ...
    async function init() {
      try {
        ...
        // Most common cause: the person hasn't allowed location access, or
        // they're using this in a context (like this preview) without GPS.
  ...
  // Step 2: once we know which stations/stops are nearby, pull live arrivals
  // and alerts for them, refreshing every 30 seconds.
  useEffect(() => {
    ...
    async function loadLiveData() {
      try {
        ...
        if (!cancelled) {
          ...
          setLiveAlerts(
            alertData.alerts.slice(0, 4).map((a) => ({
              id: a.id,
              severity: "delay",
              line: a.lines[0] || "—",
              text: a.text,
        ...
        // Backend not deployed yet, or a feed hiccup — keep showing whatever we have.
  ...
  // Step 3: free-text search across the FULL citywide list (used in the
  // "Anywhere in NYC" tab) — debounced so it doesn't fire on every keystroke.
  useEffect(() => {
      ...
      <div style={{ padding: "14px 18px 0" }}>
        {tab === "nearby" && (
          <>
            {locationStatus === "loading" && (
              <p style={{ fontSize: 13, color: "#73737a" }}>Finding what's near you…</p>
            )}
            {locationStatus === "denied" && (
              <p style={{ fontSize: 13, color: "#73737a", lineHeight: 1.5 }}>
                Location access is off, so we can't show what's nearby. Turn it on in your
                browser/phone settings, or use the search bar above to find a station or stop by name.
              </p>
            )}
            {locationStatus === "error" && (
              <p style={{ fontSize: 13, color: "#73737a", lineHeight: 1.5 }}>
                Couldn't get your location just now. You can still search any station or stop above.
              </p>
            )}
            {locationStatus === "granted" && nearbyStations.length === 0 && nearbyBusStops.length === 0 && (
              <p style={{ fontSize: 13, color: "#73737a" }}>
                Nothing within ¼ mile of you right now — try the search bar, or check Transit+ for a wider radius.
              </p>
            )}
            {nearbyStations.map((s) => (
              <StopRow key={s.id} stop={s} locked={false} onLocked={() => {}} saved={!!saved[s.id]} onToggleSave={toggleSave} arrivalsData={liveArrivals} />
            ))}
            {nearbyBusStops.map((s) => (
              <StopRow key={s.id} stop={s} locked={false} onLocked={() => {}} saved={!!saved[s.id]} onToggleSave={toggleSave} arrivalsData={liveArrivals} />
            ))}
          </>
        )}
        ...
        {tab === "explore" && (
          <>
            ...
            {query.trim().length < 2 && (
              <p style={{ fontSize: 13, color: "#73737a" }}>Type a station or bus stop name above to search all of NYC.</p>
            )}
        ...
        {tab === "routes" && (
          <>
            ...
            <p style={{ fontSize: 11.5, color: "#5a5a5f", marginTop: 10, lineHeight: 1.5 }}>
              Free routes are limited to stations within your nearby radius. Routes
              reaching further into the city need Transit+.
            </p>
          </>
        )}
      </div>
